...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateAreaSelector() {
        if (areaSelector && typeof questionBank !== 'undefined' && questionBank.getAllQuestions) {
            try {
                ...
                console.log("Dashboard: Area selector populated with areas:", areas);
            } catch (e) {
                console.error("Dashboard: Error populating area selector:", e);
                areaSelector.options.add(new Option(translate("error_loading_areas") || "Error loading areas", ""));
            }
        } else if (areaSelector) {
            console.warn("Dashboard: questionBank not ready or areaSelector element not found.");
    ...
    // após todos os bancos serem adicionados em banco-questoes.js
    setTimeout(() => {
        if (typeof questionBank !== 'undefined') {
            populateAreaSelector();
        } else {
            console.error("Dashboard: questionBank is not defined after timeout. Areas cannot be populated.");
    ...
    if (startButton && durationInput && numQuestionsInput && areaSelector) {
        startButton.addEventListener('click', () => {
            ...
            if (selectedAreasValues.length > 0 && !selectedAreasValues.includes("all")) {
                ...
                console.log("Dashboard: 'All Areas' selected or no specific area, omitting 'areas' param for full scope.");
            ...
            let quizUrl = `../Quiz/quiz-basic.html?duration=${duration}&num_questions=${numQuestions}`;
            if (areasQueryParam) {
                quizUrl += areasQueryParam;
            }

            console.log("Redirecting to Quiz Basic with URL:", quizUrl);
            window.location.href = quizUrl;
        });
    } else {
        console.error("Dashboard: Error! Could not find one or more required elements for Pro Simulation setup on the dashboard page.");
    }
});